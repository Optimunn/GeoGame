...
mod process;
use process::{In, Continent, GameLogic, Country};
mod uipart;
...
fn main() -> Result<(), slint::PlatformError> {
    ...
    let serialized_countries = In::read_from_file("data/country.json").unwrap();
    
    let continent = [Continent::Europe];
    let filtered_cont = In::filter_by_continents(serialized_countries, &continent);
...
fn update_country(main_window: &MainWindow, countries: &[Country], _random_number: usize) -> ModelRc<ButtonData> {
    ...
    let out4 = In::get_random_countries(countries, 4);
